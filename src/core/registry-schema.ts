...
export interface RegistryRecord {
  ...
  sessionFile: string | null; parentSessionFile?: string | null; leafId: string | null; cwd: string; sessionName: string | null;
...
const keys = new Set(["schemaVersion","instanceId","sequence","pid","processIdentity","startedAt","heartbeatAt","activityAt","sessionId","sessionFile","parentSessionFile","leafId","cwd","sessionName","mode","provider","model","thinking","activity","activeTools","capabilities"]);
...
export function validateRecord(value: unknown): value is RegistryRecord {
  ...
  if (Object.keys(v).some(k => !keys.has(k)) || (Object.keys(v).length !== keys.size && !(Object.keys(v).length === keys.size - 1 && !("parentSessionFile" in v)))) return false;
  ...
  if ("parentSessionFile" in v && !nullable(v.parentSessionFile)) return false;
