...
export function validateRecord(value: unknown): value is RegistryRecord {
  ...
  if (
    ...
    (Object.keys(v).length !== keys.size &&
      !(Object.keys(v).length === keys.size - 1 && !("parentSessionFile" in v)))
  )
    return false;
  if (
    v.schemaVersion !== 1 ||
    typeof v.instanceId !== "string" ||
    !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(v.instanceId)
  )
    return false;
  if (
  ...
  if (
    ...
    !activities.has(v.activity as Activity) ||
    !Array.isArray(v.capabilities) ||
    v.capabilities.length > 100 ||
    !v.capabilities.every(str)
  )
    return false;
  if (!Array.isArray(v.activeTools) || v.activeTools.length > 100 || !v.activeTools.every(tool))
    return false;
  return true;
}
