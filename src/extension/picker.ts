...
function columnWidths(rows: PageRow[], width: number) {
  const widest = (key: "project" | "name" | "meta" | "pid") =>
    rows.reduce((max, row) => Math.max(max, visibleWidth(terminalText(row[key] ?? ""))), 0);
  ...
  const meta =
    width >= 76
      ? Math.min(widest("meta"), Math.max(0, available - Math.min(24, widest("name"))))
      : 0;
...
export function rowColumns(
  row: PageRow,
  width: number,
  columns = columnWidths([row], width),
): string {
  const clean = {
    project: terminalText(row.project),
    name: terminalText(row.name),
    meta: terminalText(row.meta),
  };
  ...
  return {
    render(width: number): string[] {
      ...
      const rows = page.rows.map((row) =>
        row.savedAt ? { ...row, meta: `saved ${relativeTime(row.savedAt, now)}` } : row,
      );
      const columnHeader: PageRow = {
        id: "",
        project: "Project",
        name: "Session",
        meta: page.tab === "Recent" ? "Last saved" : "Status · Model",
        ...(page.tab === "Running" ? { pid: "PID" } : {}),
      };
      ...
      const stamp =
        page.updatedAt === undefined
          ? "Not loaded"
          : `Updated ${relativeTime(new Date(page.updatedAt).toISOString(), now)} · snapshot`;
      const status = page.notice;
      const actionHints = loading
        ? `Esc cancel · Tab / ← → cancel & switch${page.rows.length ? " · ↑↓ previous rows" : ""}`
        : page.actions
            .map(
              (action) =>
                ({
                  "Show more": "m More",
                  Refresh: "r Refresh",
                  "Discovery details": "c Discovery details",
                  Close: "Esc Close",
                })[action] ?? action,
            )
            .join(" · ");
      ...
      const header = loading
        ? [tabs, ...progress.slice(0, height >= 10 ? 3 : 2).map(terminalText)]
        : [tabs, terminalText(page.summary), terminalText(height < 8 ? (status ?? stamp) : stamp)];
      if (loading && height >= 10)
        header.push(
          terminalText(
            page.updatedAt === undefined
      ...
      if (!loading && page.warnings.length && height >= 12)
        header.push(
          theme.fg(
            "warning",
            `${page.warnings.length} scan warning${page.warnings.length === 1 ? "" : "s"} · c details`,
          ),
        );
      const compactHints = loading
        ? "Esc cancel · Tab switch"
        : `${page.actions.includes("Show more") ? "m " : ""}r c Esc`;
      const footer = new Text(height < 10 ? compactHints : actionHints, 0, 0)
        .render(width)
        .slice(0, Math.max(1, height - header.length - 2))
        .map((line) => theme.fg("muted", line));
      if (height >= 10 && !loading)
        footer.push(theme.fg("dim", "Tab / ← → tabs · ↑↓ navigate · Enter details"));
      if (rowWidth >= 76 && height >= 14 && (!loading || page.rows.length))
        header.push(theme.fg("dim", "  " + rowColumns(columnHeader, rowWidth, columns)));
      ...
      if (!page.rows.length) {
        const explanation = loading
          ? [
              ...
              ...(page.updatedAt !== undefined
                ? ["Your previous snapshot is empty; it is kept until this refresh succeeds."]
                : []),
              page.tab === "Recent"
                ? "Scanning saved files to find the newest 15. The number of files is not known in advance."
              ...
              ...(now - loading.startedAt >= 10_000
                ? [
                    "Still working. Large histories or slow disks can take longer. You can cancel and retry with r.",
                  ]
                : []),
            ].join("\n\n")
        ...
        const list = new SelectList(
          rows.map((row) => ({ value: row.id, label: rowColumns(row, rowWidth, columns) })),
          pageSize,
          {
    ...
    handleInput(data: string) {
      ...
      if ((["tab", "shift+tab", "left", "right"] as const).some((key) => matchesKey(data, key)))
        done(page.tab === "Running" ? "Recent" : "Running");
      else if (!loading && kb.matches(data, "tui.select.confirm")) {
        if (page.rows[selected]) done(page.rows[selected]!.id);
        ...
        if (kb.matches(data, "tui.select.down"))
          selected = Math.min(page.rows.length - 1, selected + 1);
        if (kb.matches(data, "tui.select.pageUp")) selected = Math.max(0, selected - pageSize);
        if (kb.matches(data, "tui.select.pageDown"))
          selected = Math.min(page.rows.length - 1, selected + pageSize);
        focus();
      }
      tui.requestRender();
    },
...
export async function selectSessionPage(
  ...
  if (ctx.mode === "tui") {
    ...
    try {
      return await ctx.ui.custom<string | undefined>((tui, theme, kb, done) => {
        tick = setInterval(() => tui.requestRender(), 15_000);
        tick.unref();
  ...
  const labels = page.rows.map((row, i) =>
    terminalText(`${i + 1}. ${row.project} · ${row.name} · ${rowMetadata(row)}`),
  );
  const choice = await ctx.ui.select(
    [
      page.tab === "Running" ? "[Running]  Recent" : "Running  [Recent]",
      page.summary,
      page.updatedAt === undefined
        ? "Not loaded"
        : `Updated ${relativeTime(new Date(page.updatedAt).toISOString(), page.now)} · snapshot`,
      ...(page.notice ? [page.notice] : []),
      ...(page.warnings.length
        ? [
            `${page.warnings.length} scan warning${page.warnings.length === 1 ? "" : "s"} · Discovery details`,
          ]
        : []),
...
export type LoadResult<T> =
  ...
  | { status: "cancelled" }
  | { status: "error" }
  | { status: "navigate"; tab: SessionsTab };

/** Keep the prior page visible. Cancellation invalidates late results and closes I/O cooperatively. */
export async function loadSessionPage<T>(
  ctx: ExtensionCommandContext,
  page: SessionPage,
  label: string,
  work: (signal: AbortSignal, report: (progress: LoadProgress) => void) => Promise<T>,
  onFocus: (id: string) => void = () => {},
  ...
  const loading: LoadingState = {
    label,
    startedAt: clock(),
    progress: { phase: "checking-running" },
  };
  let ended = false;
  ...
  const run = async (): Promise<LoadResult<T>> => {
    try {
      ...
      const value = await work(controller.signal, report);
      return controller.signal.aborted ? { status: "cancelled" } : { status: "ok", value };
    } catch {
      return controller.signal.aborted ? { status: "cancelled" } : { status: "error" };
    }
  };
  if (ctx.mode === "tui") {
    ...
    try {
      return (
        (await ctx.ui.custom<LoadResult<T>>((tui, theme, kb, done) => {
          const finish = (result: LoadResult<T>) => {
            if (!ended) {
              ended = true;
              done(result);
            }
          ...
          const component = pageComponent(
            ...
            (choice) => {
              controller.abort();
              finish(
                choice === "Running" || choice === "Recent"
                  ? { status: "navigate", tab: choice }
                  : { status: "cancelled" },
              );
            },
            onFocus,
  ...
  if (ctx.mode === "rpc") {
    const title = [
      label,
      "Scanning metadata; results appear when the scan completes. This dialog does not show live progress.",
      ...(page.updatedAt === undefined ? [] : [`Previous results · ${page.summary}`]),
      ...page.rows.map((row) => `${row.project} · ${row.name} · ${rowMetadata(row)}`),
    ]
      .map(terminalText)
      .join("\n");
    ...
    try {
      return await Promise.race([
        run(),
        prompt.then((): LoadResult<T> => ({ status: "cancelled" })),
      ]);
    } finally {
      ended = true;
      controller.abort();
...
export async function showDiscoveryDetails(
  ctx: ExtensionCommandContext,
  page: SessionPage,
): Promise<void> {
  ...
  await ctx.ui.custom<void>((tui, theme, kb, done) => {
    ...
    return {
      render(width) {
        ...
        maximum = Math.max(0, lines.length - capacity);
        offset = Math.min(offset, maximum);
        const position = maximum
          ? ` · ${offset + 1}-${Math.min(offset + capacity, lines.length)}/${lines.length}`
          : "";
        const headings = ["Scan results", "Latest attempt", "What to do", `How ${page.tab} works`];
        const body = lines
          .slice(offset, offset + capacity)
          .map((line) => (headings.includes(line.trim()) ? theme.fg("accent", line) : line));
        return [theme.fg("accent", title), ...body, `Esc / Enter back · ↑↓ scroll${position}`].map(
