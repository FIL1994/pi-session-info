        ...
        for (;;) {
          ...
          const page: SessionPage = {
            ...
            rows: tab === "Running" ? overview.sessions.map((row) => ({
              ...
              savedAt: row.modifiedAt, meta: `saved ${relativeTime(row.modifiedAt, now)} · ${row.parentSessionFile ? "derived" : "ancestry unknown"}`,
            })),
            actions: [...(tab === "Recent" ? (recent.sessions.length > limit ? ["Show more"] : []) : []), "Refresh", "Discovery details", "Close"],
          ...
          if (choice === "Running" || choice === "Recent") {
            ...
            const next = recent.sessions[limit]; if (next) selected.Recent = `saved:${next.sessionId}`;
            limit += 10;
          } else if (tab === "Running") {
