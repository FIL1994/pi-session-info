...
export async function readHistory(
    ...
    try {
      ...
      try {
        ...
        if (!before.isFile() || (process.getuid && before.uid !== process.getuid())) {
          note("unsafe or non-regular files skipped");
          return;
        }
        ...
        if (end < 0) {
          note("invalid or oversized session headers skipped");
          return;
        }
        let header: unknown;
        try {
          header = JSON.parse(head.subarray(0, end).toString("utf8"));
        } catch {
          note("invalid or oversized session headers skipped");
          return;
        }
        if (
          ...
          note("invalid or unsupported session headers skipped (requires v3)");
          return;
        }
        ...
        while (offset < tail.length) {
          ...
          if (newline < 0) {
            note("files with incomplete final records (record skipped)");
            break;
          }
        ...
        if (
          ...
          note("files changed during reading and skipped");
          return;
        }
        ...
        if (
          !old ||
          row.modifiedAt > old.modifiedAt ||
          (row.modifiedAt === old.modifiedAt && path < old.sessionFile)
        )
      ...
      check();
      note("unreadable, unsafe, or changed session files skipped");
    }
  }
  ...
  async function scan(root: string, descend: boolean): Promise<void> {
    ...
    try {
      ...
      if ((await realpath(root)) !== root || !(await lstat(root)).isDirectory()) {
        note("unsafe or non-directory history paths skipped");
        return;
      }
      ...
      if ((error as NodeJS.ErrnoException).code !== "ENOENT")
        note("history directories could not be read");
    }
  }
  ...
  if (limited)
    warnings.push(
      `History scan limit reached (${MAX_ENTRIES} entries / ${MAX_FILES} files); newest-session coverage may be incomplete.`,
    );
  report("sorting");
  return {
