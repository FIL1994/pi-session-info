...
): "matched" | "unavailable" | "unknown" {
  if (!child.parentSessionFile) return "unknown";
  return sessions.some(
    (candidate) => fileKey(candidate.sessionFile) === fileKey(child.parentSessionFile!),
  )
    ? "matched"
    : "unavailable";
}
...
export function recentSessions(
  ...
  const files = new Set(
    live.sessions.flatMap((row) => (row.sessionFile ? [fileKey(row.sessionFile)] : [])),
  );
...
export function savedDetails(row: SavedSession, snapshot?: readonly SavedSession[]): string[] {
  return [
    row.name ?? "Saved session",
    `Directory: ${row.cwd}`,
    `Session: ${row.sessionId}`,
    `File: ${row.sessionFile}`,
    `Last saved (file modified): ${row.modifiedAt}`,
    `Derived from: ${row.parentSessionFile ?? "Unknown (no parent metadata)"}`,
    ...(snapshot && row.parentSessionFile
      ? [
          parentMatch(row, snapshot) === "matched"
