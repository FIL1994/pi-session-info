...
const saved = Array.from({ length: 25 }, (_, i) => ({
  sessionId: `saved-${i}`,
  sessionFile: `/synthetic/${i}.jsonl`,
  cwd: "/synthetic/project",
  name: `Task ${i}`,
  modifiedAt: new Date(1_000_000 - i * 1000).toISOString(),
}));
...
test("Recent starts at fifteen, Show more adds ten, and history is cached until refresh", async () => {
  ...
  const app = setup(["Recent", "Show more", "Refresh", "Running", "Close"], false, async () => {
    reads++;
    return { sessions: saved, warnings: [] };
  });
  await app.run();
  expect(reads).toBe(2);
  expect(app.dialogs[1]?.title).toContain("15 of 25");
  expect(app.dialogs[1]?.choices.slice(15)).toEqual([
    "Show more",
    "Refresh",
    "Discovery details",
    "Close",
...
test("Running does not read history", async () => {
  let reads = 0;
  const app = setup(["Refresh", "Close"], false, async () => {
    reads++;
...
test("Recent details are read-only metadata and escape control bytes", async () => {
  const app = setup(["Recent", "first", "Back", "Close"], false, async () => ({
    sessions: [{ ...saved[0]!, name: "Task\u001b[31m" }],
    warnings: [],
  }));
  await app.run();
  ...
  expect(app.dialogs[2]?.choices).toEqual(["Back"]);
...
test("Recent excludes the viewer's session before selecting the first fifteen", async () => {
  const app = setup(["Recent", "Close"], false, async () => ({
    sessions: saved.slice(0, 16),
    warnings: [],
  }));
  app.ctx.sessionManager = {
    getSessionId: () => "saved-0",
    getSessionFile: () => "/synthetic/0.jsonl",
...
test("picker offers read-only details and refresh", async () => {
  const app = setup(["first", "Back", "Refresh", "Close"]);
  await app.run();
  expect(app.dialogs[1]?.title).toContain("Directory:");
  expect(app.dialogs[1]?.choices).toEqual(["Back"]);
...
test("invalid args, no UI, and discovery errors are safe", async () => {
  const bad = setup();
  await bad.run("extra");
  expect(bad.scans()).toBe(0);
  expect(bad.notifications).toEqual(["Usage: /sessions"]);
  const headless = setup();
  headless.ctx.hasUI = false;
  await headless.run();
  expect(headless.scans()).toBe(0);
  const failed = setup([], true);
  await failed.run();
  expect(failed.dialogs[0]?.title).toContain("Could not load sessions");
});
