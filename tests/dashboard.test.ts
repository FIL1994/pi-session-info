...
async function dashboard(
  deps: SessionsDeps,
  keys: string[][],
  onLoading?: (component: Component, output: string) => void,
) {
            ...
            const component = factory(
              ...
              { fg: (_: string, s: string) => s },
              {
                matches: (data: string, action: string) =>
                  actions[action] ? matchesKey(data, actions[action]) : false,
              },
              resolve,
            );
            const output = component.render(120).join("\n");
            if (
              output.includes("Refreshing running sessions") ||
              output.includes("Loading saved sessions") ||
              output.includes("Refreshing saved sessions")
            ) {
              loading.push(output);
              onLoading?.(component, output);
              return;
            }
            pages.push(output);
...
test("Running selection survives details, reordering refresh, and a tab round trip", async () => {
  let scans = 0;
  const initial = demoOverview();
  initial.sessions = initial.sessions.slice(0, 2);
  const reordered = { ...initial, sessions: [...initial.sessions].reverse() };
  const result = await dashboard(
    {
      overview: () => (++scans === 1 ? initial : reordered),
      history: async () => ({ sessions: [], warnings: [] }),
    },
    [["\u001b[B", "\r"], ["r"], ["\t"], ["\t"], ["\u001b"]],
  );
  for (const index of [1, 2, 4])
    expect(result.pages[index]?.split("\n").find((s) => s.startsWith("→"))).toContain(
      "Review workspace",
    );
...
test("refresh failure retains rows, selection, and prior timestamp with retry notice", async () => {
  ...
  const result = await dashboard(
    {
      overview: () => {
        if (++scans > 1) throw Error("private failure");
        return demoOverview();
      },
    },
    [["\u001b[B", "r"], ["\u001b"]],
...
test("Show more focuses the first new identity and a failed history refresh retains rows", async () => {
  ...
  const sessions = Array.from({ length: 21 }, (_, i) => ({
    sessionId: `saved-${i}`,
    sessionFile: `/fixture/${i}`,
    cwd: "/fixture",
    name: `Task-${i}`,
    modifiedAt: new Date(1000 - i).toISOString(),
  }));
  const result = await dashboard(
    {
      overview: demoOverview,
