...
function harness(options: { failWrite?: boolean; noIdentity?: boolean; hasUI?: boolean } = {}) {
  ...
  const ctx = {
    ...
    thinkingLevel: "high",
    get model() {
      return { provider: "fixture", id: model };
    },
    isIdle: () => idle,
    sessionManager: {
  ...
  const pi = {
    on: (event: string, handler: (event: Record<string, unknown>, ctx: ExtensionContext) => void) =>
  ...
  registerLifecycle(pi, {
    ...
    pid: 10,
    dir: "/unused-fixture",
    identity: () => (options.noIdentity ? null : "boot:123"),
    ...
    clearTimeout: ((handle: { id: number }) =>
      timers.delete(handle.id)) as unknown as typeof clearTimeout,
    write: (_dir, record) => {
...
test("activation publishes real metadata immediately; idle heartbeat doesn't invent activity", () => {
  ...
  h.emit("session_start");
  expect(h.last().model).toBe("fixture-model");
  expect(h.last().activity).toBe("idle");
  expect(h.last().cwd).toBe("/fixture/project");
  expect(h.last().thinking).toBe("high");
  const activityAt = h.last().activityAt;
  h.advance(5_000);
  expect(h.last().sequence).toBe(2);
  expect(h.last().activityAt).toBe(activityAt);
...
test("tool bursts coalesce, preserve concurrent calls and exclude arguments/results", () => {
  ...
  h.emit("tool_execution_start", {
    toolCallId: "a",
    toolName: "read",
    args: { secret: "never-record-this" },
  });
  h.emit("tool_execution_start", { toolCallId: "b", toolName: "bash" });
  expect(h.writes).toHaveLength(1);
  h.advance(250);
  expect(h.last().activeTools).toHaveLength(2);
  h.emit("tool_execution_end", { toolCallId: "a", result: "never-record-this" });
  h.advance(250);
  expect(h.last().activity).toBe("tool");
  expect(h.last().activeTools).toEqual([{ id: "b", name: "bash" }]);
  h.emit("tool_execution_end", { toolCallId: "b" });
  h.advance(250);
  expect(h.last().activity).toBe("working");
  expect(JSON.stringify(h.writes)).not.toContain("never-record-this");
  h.emit("session_shutdown");
});
...
test("retry, UI waiting, compaction failure and host sampling don't leave false idle or stuck working", () => {
  ...
  h.advance(250);
  expect(h.last().activity).toBe("waiting-user");
...
test("model/name refresh and heartbeat sampling update activity time only on changes", () => {
  const h = harness();
  h.emit("session_start");
  const old = h.last().activityAt;
  h.advance(100);
  h.model("new-model");
  h.emit("model_select");
  h.advance(250);
  expect(h.last().model).toBe("new-model");
  expect(h.last().activityAt).not.toBe(old);
  h.name("new name");
  h.advance(5_000);
  expect(h.last().sessionName).toBe("new name");
  h.emit("session_shutdown");
});
...
test("shutdown cancels pending writes and late events; reactivation has a new identity", () => {
  const h = harness();
  h.emit("session_start");
  const first = h.last().instanceId;
  h.model("next");
  h.emit("model_select");
  h.emit("session_shutdown");
  h.emit("session_shutdown");
  h.emit("tool_execution_start", { toolCallId: "late", toolName: "read" });
  h.advance(10_000);
  expect(h.writes).toHaveLength(1);
  expect(h.removed).toEqual([first]);
  h.session("new-session");
  h.emit("session_start");
  expect(h.last().instanceId).not.toBe(first);
  expect(h.last().sessionId).toBe("new-session");
  h.emit("session_shutdown");
});
