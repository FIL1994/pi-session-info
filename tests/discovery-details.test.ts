...
const page: SessionPage = { tab: "Running", summary: "2 running · 2 connected", warnings: [], rows: [], actions: [], empty: "", now: 1000, updatedAt: 1000 };
...
test("only the known static limitation is moved to help; other warnings survive", () => {
  ...
  expect(scanWarnings([PROCESS_DISCOVERY_LIMITATION, "Unknown warning", "Unknown warning", "Scan limit reached"]))
...
test("real warnings come first, with next steps, and untrusted text is escaped", () => {
  const text = discoveryText({ ...page, warnings: ["History: unreadable\u001b[31m files", "Scan limit reached"] });
...
test("unloaded and failed attempts are never described as successful scans", () => {
  ...
  expect(discoveryText({ ...page, notice: "Refresh failed · previous snapshot retained" })).toContain("Latest attempt\nRefresh failed");
});
