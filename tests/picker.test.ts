...
const page = (): SessionPage => ({
  ...
  now: 1000,
  updatedAt: 1000,
  summary: "20 running · 18 connected",
  empty: "No sessions",
  warnings: ["Long warning ".repeat(80)],
  actions: ["Refresh", "Discovery details", "Close"],
...
function host(
  script: (component: Component) => void | Promise<void>,
  height = 24,
  requestRender = () => {},
) {
  return {
...
test("tabs, fixed summaries/actions, responsive Unicode rows, and injected selection", async () => {
  ...
  expect(
    await selectSessionPage(ctx, p, (id) => {
      focus = id;
    }),
  ).toBe("id-11");
  ...
  await selectSessionPage(
    host((component) => {
      for (const width of [46, 60, 79, 80, 100, 160, 240]) {
        ...
        const position = (line: string, text: string) =>
          visibleWidth(line.slice(0, line.indexOf(text)));
...
test("Recent columns stay content-sized and never show a PID column", async () => {
  ...
  await selectSessionPage(
    host((component) => {
      ...
      expect(lines.join("\n")).not.toContain("PID");
      expect(lines.find((line) => line.includes("Saved session"))).toContain(
        "Saved session  saved just now",
      );
      component.handleInput?.("\u001b");
    }),
    p,
...
test("RPC labels retain the separate PID", async () => {
  const p = page();
  p.rows = [{ id: "one", project: "project", name: "Session", meta: "Idle", pid: "PID 1234567" }];
  const ctx = {
    mode: "rpc",
    ui: {
...
test("TUI tabs, escape, and action shortcuts return stable values", async () => {
  ...
  expect(
    await selectSessionPage(
      ...
      { ...page(), tab: "Recent", actions: ["Refresh", "Discovery details", "Close"] },
      () => {},
    ),
  ).toBeUndefined();
  expect(calls).toBe(1);
...
test("short terminals retain tabs and actions without exceeding available height", async () => {
  const p = {
    ...page(),
    tab: "Recent" as const,
    actions: ["Show more", "Refresh", "Discovery details", "Close"],
  };
...
test("loading renders previous rows, propagates cancellation, and ignores late completion", async () => {
  ...
  const result = await loadSessionPage(
    host(async (component) => {
...
test("load success and failure settle without leaking rejected work", async () => {
  const ctx = host(() => {});
  expect(await loadSessionPage(ctx, page(), "Loading", async () => 42)).toEqual({
    status: "ok",
    value: 42,
  });
  expect(
    await loadSessionPage(ctx, page(), "Loading", async () => {
      throw Error("secret");
    }),
...
test("switching tabs before the first load starts never invokes the reader", async () => {
  let called = false;
  const result = await loadSessionPage(
    host((component) => component.handleInput?.("\t")),
    page(),
    "Loading",
    async () => {
      called = true;
...
test("Recent first load shows real progress, elapsed time, and no misleading empty results", async () => {
  ...
  const { updatedAt: _updatedAt, ...initial } = page();
  await loadSessionPage(
    host(async (component) => {
      ...
      expect(scanning.join("\n")).toContain(
        "86 files checked · 72 saved sessions found · 4 folders visited",
      );
    ...
    { ...initial, tab: "Recent", rows: [], summary: "Recent sessions · 0 of 0", clock: () => now },
    "Loading saved sessions…",
    (_signal, update) => {
      report = update;
      started();
      ...
      await loadSessionPage(
        host((component) => {
          for (const width of [16, 32, 80, 120]) {
            ...
            for (const line of lines) expect(visibleWidth(line)).toBeLessThanOrEqual(width);
            expect(lines.join("\n")).toContain("Esc");
            if (height >= 12 && width === 120)
              expect(lines.join("\n")).toContain("Previous results · Updated just now");
          }
          component.handleInput?.("\u001b");
        }, height),
        { ...page(), tab: "Recent", rows, clock: () => 1000 },
    ...
    host(
      async (c) => {
        ...
        for (let i = 0; i < 1000; i++)
          report({ phase: "scanning", files: i, sessions: i, directories: 1 });
...
test("RPC cancellation aborts work and retains a metadata snapshot in the loading dialog", async () => {
  ...
  const result = await loadSessionPage(ctx, page(), "Loading", (s) => {
    signal = s;
    return new Promise(() => {});
  });
      ...
      host((component) => {
        ...
        if (warnings.length)
          expect(output).toContain(
            `${warnings.length} scan warning${warnings.length === 1 ? "" : "s"} · c details`,
          );
