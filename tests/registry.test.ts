...
const record = (id = "123e4567-e89b-12d3-a456-426614174000") => ({
...
test("validates strict records and rejects unknown fields", () => {
  expect(validateRecord(record())).toBe(true);
  expect(validateRecord({ ...record(), secret: "no" })).toBe(false);
});
test("writes atomically and skips malformed records", () => {
  const dir = mkdtempSync(join(tmpdir(), "registry-test-"));
  try {
    writeRecord(dir, record());
    writeFileSync(join(dir, "123e4567-e89b-12d3-a456-426614174001.json"), "{}");
    const got = readRecords(dir);
...
test("resolves explicit paths and only valid private XDG runtime dirs", () => {
  ...
  try {
    ...
    expect(resolveRegistryDir({ XDG_RUNTIME_DIR: root }, "/home/u")).toBe(
      join(root, "pi-session-info"),
    );
    for (const path of ["relative", join(root, "missing")])
      expect(resolveRegistryDir({ XDG_RUNTIME_DIR: path }, "/home/u")).toBe(
        "/home/u/.cache/pi-session-info/run",
      );
...
test("registry permissions, missing directory, atomic replacement and idempotent removal", () => {
  const root = mkdtempSync(join(tmpdir(), "registry-safe-"));
  const dir = join(root, "run");
  try {
    expect(readRecords(dir)).toEqual({ records: [], warnings: [] });
    expect(existsSync(dir)).toBe(false);
    removeRecord(dir, record().instanceId);
    writeRecord(dir, record());
    writeRecord(dir, { ...record(), sequence: 2 });
    expect(statSync(dir).mode & 0o777).toBe(0o700);
    expect(statSync(join(dir, `${record().instanceId}.json`)).mode & 0o777).toBe(0o600);
    expect(readRecords(dir).records[0]?.sequence).toBe(2);
...
test("unsafe directories and symlinks cannot be followed or created through", () => {
  ...
  try {
    ...
    expect(() => writeRecord(join(link, "child"), record())).toThrow();
    ...
    expect(() => writeRecord(join(target, "child"), record())).toThrow();
    expect(existsSync(join(target, "child"))).toBe(false);
...
test("untrusted records are bounded, nonblocking, and never leak file descriptors", () => {
  ...
  const ids = Array.from(
    { length: 5 },
    (_, n) => `123e4567-e89b-12d3-a456-${String(n).padStart(12, "0")}`,
  );
  try {
...
test("schema rejects versions, invalid dates, content fields, and oversized collections", () => {
  for (const extra of [
    { schemaVersion: 2 },
    { heartbeatAt: "2026-02-30T00:00:00.000Z" },
    { mode: "other" },
    { processIdentity: "" },
    { pid: 0 },
    { sequence: -1 },
...
test("oversized writes and unsafe record removal fail without touching unrelated data", () => {
  ...
  try {
    ...
    expect(() =>
      writeRecord(child, { ...record(), capabilities: Array(100).fill("x".repeat(4096)) }),
    ).toThrow("too large");
    expect(existsSync(child)).toBe(false);
    const target = join(root, "unrelated");
    writeFileSync(target, "keep", { mode: 0o600 });
    const link = join(root, `${record().instanceId}.json`);
    symlinkSync(target, link);
    expect(() => removeRecord(root, record().instanceId)).toThrow();
...
test("identity rejects dead states and malformed ticks, handles parentheses in comm", () => {
  ...
  try {
    ...
    for (const state of ["S", "Z", "X", "x"]) {
      writeFileSync(
        join(root, "7/stat"),
        `7 (pi (with spaces)) ${[state, ...Array(18).fill("0"), "42"].join(" ")}`,
      );
      expect(readProcessIdentity(7, root)).toBe(state === "S" ? "boot:42" : null);
    }
    writeFileSync(
      join(root, "7/stat"),
      `7 (pi) ${["S", ...Array(18).fill("0"), "invalid"].join(" ")}`,
    );
    expect(readProcessIdentity(7, root)).toBeNull();
    expect(readProcessIdentity(-1, root)).toBeNull();
    expect(readProcessIdentity(8, root)).toBeNull();
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
